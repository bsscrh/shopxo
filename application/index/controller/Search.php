...
//            var_dump($this->params['category_id']);
//            var_dump($this->nav_header);
            $nav_header = $this->nav_header;
            foreach ($nav_header as $key => $value){
                foreach ($value['items'] as $key2 => $value2){
                    if($this->params['category_id'] == $value2['value']){
                        $nav_header[$key]['name'] = $value2['name'];
                    }
                }
            }
            $this->assign('nav_header', $nav_header);
            // 商品分类
